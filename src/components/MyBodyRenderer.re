...
let rec jsTreeToReason = (jsChild: t) =>
  ...
  | "[object Object]" =>
    ...
    let children =
      switch (Js.Undefined.toOption(jsChild.children)) {
...
let rec renderChild = (~keepNewlines=false, parentTag, index: int, child) => {
  ...
  | Element(tag, htmlprops, children) =>
    ...
    | "p" =>
      <Html.P key> {renderChildren(~keepNewlines, tag, children)} </Html.P>
    ...
    | "li" =>
      <Html.Li key> {renderChildren(~keepNewlines, tag, children)} </Html.Li>
    | "blockquote" =>
      <Html.BlockQuote key>
        {renderChildren(~keepNewlines, tag, children)}
      </Html.BlockQuote>
