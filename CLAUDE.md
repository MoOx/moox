...
| Native build config                | `app.json`, `metro.config.cjs`, `plugins/`    |
| The prebuilt iOS project           | `ios/` (built with fastlane, not EAS)         |
| The apps index (`/apps`)           | `src/routes.web/apps.index.tsx` + `AppHero`   |
| App landing pages (`/apps/<slug>`) | `src/routes.web/apps.$slug.tsx` + `AppLandingPage` |
| An app's privacy policy            | `src/routes.web/apps.$slug_.privacy.tsx`      |
